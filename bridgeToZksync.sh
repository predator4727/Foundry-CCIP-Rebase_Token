...
cast send ${ZKSYNC_REBASE_TOKEN_ADDRESS} "grantMintAndBurnRole(address)" ${ZKSYNC_POOL_ADDRESS} --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account default
...
cast send ${ZKSYNC_POOL_ADDRESS}  --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account default "applyChainUpdates(uint64,bool,bytes,bytes,(bool,uint128,uint128),(bool,uint128,uint128)[])" "[(${SEPOLIA_CHAIN_SELECTOR},(true),$(cast abi-encode "f(address)" ${SEPOLIA_POOL_ADDRESS}),$(cast abi-encode "f(address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS}),(false,0,0),(false,0,0))]"
