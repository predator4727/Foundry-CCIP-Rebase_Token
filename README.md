...
2. Rebase token - `balanceOf` function is dynamic to show the changing balance over time.
    - Balance increasies linearly over time 
    - Mint token to users every time they perform an action (minting, burning, bridging, transfering)
3. Interest rate:
    - Individually set an interest rate to each user based on global interest rate of of the protocol at the time user deposits into the vault.
...
```
git clone https://github.com/predator4727/foundry-cross-chain-rebase-token
